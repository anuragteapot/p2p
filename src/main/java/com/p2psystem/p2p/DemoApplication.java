		...
		while (true) {
			...
			try {
				...
				if (data[0].equals("find_successor")) {
					...
					// update finger table
					// System.out.println("node port "+node.successorId+ " "+node.successorPort+"
					// "+node.nodeId);
...
class node {
	...
	public static int find_predecessor(int id) {
		...
		while (!(flag)) {
			if (curPredecessor == node.nodeId) {
				...
				suc = (curPredecessor == node.nodeId) ? node.successorId
						: Integer.parseInt(
								Request.makeRequest(curPredecessor1, "successor:1222" + ":" + id % 32 + ":0"));
		...
		System.out.println(
				"****Predecessor of node i.e " + id + " is " + Chord.SHA1("localhost:" + curPredecessor1) + "****");
	...
	public static String find_successor(int id, int port, int flag) {
		...
		System.out.println(
				"****Successor Of it's Predecessor i.e" + Chord.SHA1("localhost:" + pred) + " is " + predSuc + "****");
		return predSuc + ":" + pred;
	}
...
public class Chord {
	...
	public static void main(String[] args) throws UnknownHostException {
		...
		while (true) {
			...
			if (choice == 1) {
				System.out.println("Host Ip is: " + InetAddress.getLocalHost().getHostAddress() + ":" + node.nodePort
						+ " and SHA1 id is " + node.nodeId);
			} else if (choice == 2) {
				System.out.println("PREDECESSOR--Ip is " + InetAddress.getLocalHost().getHostAddress() + ":"
						+ node.predecessorPort + " SHA1 Id is " + node.predecessorId);
				System.out.println("SUCCESSOR--Ip is " + InetAddress.getLocalHost().getHostAddress() + ":"
						+ node.successorPort + " SHA1 Id is " + node.successorId);
			} else if (choice == 3) {
				File folder = new File("./nodeFile");
				File[] allFiles = folder.listFiles();
				int count = 0;
				...
				for (int i = 0; i < 5; i++) {
					System.out.println(Fingers.table[i][0] + "		" + Fingers.table[i][1] + "		"
							+ Fingers.table[i][2] + "		" + Fingers.table[i][3]);
				}
			} else if (choice == 5) {
				...
				for (int i = 0; i < 5; i++) {
					...
					Request.makeRequest(p,
							"update:" + node.nodeId + ":" + i + ":" + node.nodePort + ":" + successorPort);
				}
